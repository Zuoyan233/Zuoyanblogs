...
Zuoyanblogs ブログプロジェクトは、<a href="https://github.com/matsuzaka-yuki/mizuki" target="_blank">Mizuki</a> テーマフレームワークのオープンソースプロジェクトを基に二次改変を施したものです。<a href="https://astro.build/" target="_blank">Astro</a> で構築された、先進的な機能と洗練されたデザインを備えた、モダンで多機能な静的ブログテンプレートです。
...
不定期に Mizuki テーマフレームワークのメインラインコンテンツを同期更新します。将来的には、フレームワーク機能をベースにカスタマイズ修正を加える予定であり、バージョン番号には `Zuoyanblogs Custom Edition (CE)` のサフィックスを付与し、Zuoyanblogs による軽度カスタマイズバージョンであることを示します。（現在、市場で流行しているウェブサイトフレームワークを猛勉強中です。時間ができるまでもうしばらくお待ちください、ううう～～～）
...
- サイトにリアルタイム日時表示を追加。日付表示は多地域フォーマットに対応。
...
⭐ ご質問やご提案がございましたら、[Issue](https://github.com/Zuoyan233/Zuoyanblogs/issues) または [Pull Request](https://github.com/Zuoyan233/Zuoyanblogs/pulls) を作成してください。また、私のウェブサイトのフィードバックページ からご連絡いただくことも可能です。このプロジェクトがお役に立ちましたら、ぜひスターを付けてください！