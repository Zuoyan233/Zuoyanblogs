import Key from "../i18nKey";
import type { Translation } from "../translation";

export const vi: Translation = {
	[Key.home]: "Trang chủ",
	[Key.about]: "Giới thiệu",
	[Key.archive]: "Lưu trữ",
	[Key.search]: "Tìm kiếm",
	[Key.sponsors]: "nhà tài trợ",
	[Key.feedback]: "Phản hồi",
	[Key.other]: "Khác",

	// Tiêu đề bảng dịch
	[Key.translateHeader]: "Chọn ngôn ngữ",

	// Trang Phản hồi - Mô-đun Liên hệ Quản trị viên Trang web
	[Key.contactMe]: "Liên hệ với Quản trị viên Trang web",
	[Key.contactEmail]: "Liên hệ với Quản trị viên Trang web qua email",
	[Key.contactSocialFriends]:
		"Liên hệ với Quản trị viên Trang web bằng cách thêm họ làm bạn bè trên mạng xã hội",
	[Key.addFriends]: "Thêm bạn bè ngay bây giờ",

	// Trang Tài trợ - Mô-đun Các Phương thức Hỗ trợ Khác
	[Key.otherSupportOptions]: "Các phương thức hỗ trợ khác",
	[Key.shareBlog]: "Chia sẻ blog của tôi với nhiều bạn bè hơn",
	[Key.leaveComment]:
		"Để lại ý kiến ​​của bạn trong phần bình luận bên dưới bài viết",
	[Key.subscribeToArticles]:
		"Đăng ký RSS và Atom để nhận các bài viết mới nhất",

	// Tiêu đề thanh điều hướng
	[Key.navLinks]: "Liên kết",
	[Key.navMy]: "Của tôi",
	[Key.navAbout]: "Giới thiệu",
	[Key.navOthers]: "Khác",

	[Key.tags]: "Thẻ",
	[Key.categories]: "Danh mục",
	[Key.recentPosts]: "Bài đăng gần đây",
	[Key.postList]: "Danh sách bài đăng",
	[Key.tableOfContents]: "Mục lục",
	[Key.tocEmpty]: "Không có mục lục",

	// Thông báo
	[Key.announcement]: "Thông báo",
	[Key.announcementClose]: "Đóng",

	[Key.comments]: "Bình luận",
	[Key.friends]: "Bạn bè",
	[Key.friendsSubtitle]: "Khám phá thêm các trang web tuyệt vời",
	[Key.friendsSearchPlaceholder]: "Tìm kiếm tên hoặc mô tả bạn bè...",
	[Key.friendsFilterAll]: "Tất cả",
	[Key.friendsNoResults]: "Không tìm thấy bạn bè phù hợp",
	[Key.friendsVisit]: "Truy cập",
	[Key.friendsCopyLink]: "Sao chép liên kết",
	[Key.friendsCopySuccess]: "Đã sao chép",
	[Key.friendsTags]: "Thẻ",
	[Key.untitled]: "Chưa có tiêu đề",
	[Key.uncategorized]: "Chưa có phân loại",
	[Key.noTags]: "Không có thẻ",

	[Key.wordCount]: "từ",
	[Key.wordsCount]: "từ",
	[Key.minuteCount]: "phút",
	[Key.minutesCount]: "phút",
	[Key.postCount]: "bài đăng",
	[Key.postsCount]: "bài đăng",

	[Key.themeColor]: "Màu chủ đề",

	[Key.lightMode]: "Sáng",
	[Key.darkMode]: "Tối",
	[Key.systemMode]: "Hệ thống",

	[Key.more]: "Xem thêm",

	[Key.author]: "Tác giả",
	[Key.publishedAt]: "Xuất bản tại",
	[Key.license]: "Giấy phép",
	[Key.anime]: "Anime",
	[Key.diary]: "Nhật ký",

	// Trang Anime
	[Key.animeTitle]: "Danh sách Anime của tôi",
	[Key.animeSubtitle]: "Ghi lại hành trình anime của tôi",
	[Key.animeStatusWatching]: "Đang xem",
	[Key.animeStatusCompleted]: "Đã hoàn thành",
	[Key.animeStatusPlanned]: "Đã lên kế hoạch",
	[Key.animeStatusOnHold]: "Đang tạm dừng",
	[Key.animeStatusDropped]: "Đã bỏ",
	[Key.animeFilterAll]: "Tất cả",
	[Key.animeYear]: "Năm",
	[Key.animeStudio]: "Studio",
	[Key.animeEmpty]: "Không có dữ liệu anime",
	[Key.animeEmptyBangumi]:
		"Vui lòng kiểm tra cấu hình Bangumi hoặc kết nối mạng",
	[Key.animeEmptyLocal]:
		"Vui lòng thêm thông tin anime vào tệp src/data/anime.ts",

	// Trang Nhật ký
	[Key.diarySubtitle]: "Chia sẻ cuộc sống mọi lúc, mọi nơi",
	[Key.diaryCount]: "nhật ký",
	[Key.diaryReply]: "Trả lời",

	[Key.diaryTips]: "Chỉ hiển thị 30 nhật ký gần nhất",
	[Key.diaryMinutesAgo]: "phút trước",
	[Key.diaryHoursAgo]: "giờ trước",
	[Key.diaryDaysAgo]: "ngày trước",

	// Trang 404
	[Key.notFound]: "404",
	[Key.notFoundTitle]: "Không tìm thấy trang",
	[Key.notFoundDescription]:
		"Rất tiếc, trang bạn đã truy cập không tồn tại hoặc đã được di chuyển.",
	[Key.backToHome]: "Quay lại Trang chủ",

	// Trình phát nhạc
	[Key.musicPlayer]: "Trình phát nhạc",
	[Key.musicPlayerShow]: "Hiện trình phát nhạc",
	[Key.musicPlayerHide]: "Ẩn trình phát nhạc",
	[Key.musicPlayerExpand]: "Mở rộng trình phát nhạc",
	[Key.musicPlayerCollapse]: "Thu gọn trình phát nhạc",
	[Key.musicPlayerPause]: "Tạm dừng",
	[Key.musicPlayerPlay]: "Phát",
	[Key.musicPlayerPrevious]: "Bài trước",
	[Key.musicPlayerNext]: "Bài tiếp theo",
	[Key.musicPlayerShuffle]: "Phát ngẫu nhiên",
	[Key.musicPlayerRepeat]: "Lặp lại tất cả",
	[Key.musicPlayerRepeatOne]: "Lặp lại một bài",
	[Key.musicPlayerVolume]: "Điều chỉnh âm lượng",
	[Key.musicPlayerProgress]: "Tiến độ phát",
	[Key.musicPlayerCover]: "Ảnh bìa",
	[Key.musicPlayerPlaylist]: "Danh sách phát",
	[Key.musicPlayerLoading]: "Đang tải...",
	[Key.musicPlayerErrorPlaylist]: "Không thể tải danh sách phát",
	[Key.musicPlayerErrorSong]:
		"Không thể tải bài hát hiện tại, thử bài tiếp theo",
	[Key.musicPlayerErrorEmpty]: "Không có bài hát nào trong danh sách phát",
	[Key.unknownSong]: "Bài hát không xác định",
	[Key.unknownArtist]: "Nghệ sĩ không xác định",

	// Trang Album
	[Key.albums]: "Album",
	[Key.albumsSubtitle]: "Bản ghi Những khoảnh khắc đẹp trong cuộc sống",
	[Key.albumsEmpty]: "Không có nội dung",
	[Key.albumsEmptyDesc]:
		"Chưa có album nào được tạo. Hãy thêm những kỷ niệm đẹp vào nhé!",

	[Key.albumsBackToList]: "Quay lại Album",

	// Trang Thiết bị
	[Key.devices]: "Thiết bị của tôi",
	[Key.devicesSubtitle]:
		"Đây là những thiết bị tôi sử dụng trong cuộc sống hàng ngày",
	[Key.devicesViewDetails]: "Xem chi tiết",
	[Key.albumsPhotoCount]: "ảnh",
	[Key.albumsPhotosCount]: "ảnh",

	// Trang Dự án
	[Key.projects]: "Dự án",
	[Key.projectsSubtitle]: "Danh mục dự án phát triển của tôi",
	[Key.projectsAll]: "Tất cả",
	[Key.projectsWeb]: "Ứng dụng Web",
	[Key.projectsMobile]: "Ứng dụng Di động",
	[Key.projectsDesktop]: "Ứng dụng Máy tính",
	[Key.projectsOther]: "Khác",
	[Key.projectTechStack]: "Tech Stack",
	[Key.projectLiveDemo]: "Bản demo trực tiếp",
	[Key.projectSourceCode]: "Mã nguồn",
	[Key.projectDescription]: "Mô tả dự án",
	[Key.projectStatus]: "Trạng thái",
	[Key.projectStatusCompleted]: "Đã hoàn thành",
	[Key.projectStatusInProgress]: "Đang tiến hành",
	[Key.projectStatusPlanned]: "Đã lên kế hoạch",
	[Key.projectsTotal]: "Tổng số dự án",
	[Key.projectsCompleted]: "Đã hoàn thành",
	[Key.projectsInProgress]: "Đang tiến hành",
	[Key.projectsTechStack]: "Thống kê Tech Stack",
	[Key.projectsFeatured]: "Các dự án nổi bật",
	[Key.projectsPlanned]: "Đã lên kế hoạch",
	[Key.projectsDemo]: "Bản demo trực tiếp",
	[Key.projectsSource]: "Mã nguồn",
	[Key.projectsVisit]: "Truy cập Dự án",
	[Key.projectsGitHub]: "GitHub",

	// Trang RSS
	[Key.rss]: "Nguồn cấp RSS",
	[Key.rssDescription]: "Đăng ký để nhận thông tin cập nhật mới nhất",
	[Key.rssSubtitle]:
		"Đăng ký qua RSS để nhận các bài viết và cập nhật mới nhất ngay lập tức",
	[Key.rssLink]: "Liên kết RSS",
	[Key.rssCopyToReader]: "Sao chép liên kết đến trình đọc RSS của bạn",
	[Key.rssCopyLink]: "Sao chép",
	[Key.rssLatestPosts]: "Bài viết mới nhất",
	[Key.rssWhatIsRSS]: "RSS là gì?",
	[Key.rssWhatIsRSSDescription]:
		"RSS (Really Simple Syndication) là một định dạng chuẩn để xuất bản nội dung được cập nhật thường xuyên. Với RSS, bạn có thể:",
	[Key.rssBenefit1]:
		"Nhận nội dung trang web mới nhất kịp thời mà không cần truy cập thủ công",

	[Key.rssBenefit2]: "Quản lý đăng ký nhiều trang web tại một nơi",
	[Key.rssBenefit3]: "Tránh bỏ lỡ các bản cập nhật và bài viết quan trọng",
	[Key.rssBenefit4]: "Tận hưởng trải nghiệm đọc sách sạch sẽ, không quảng cáo",
	[Key.rssHowToUse]:
		"Khuyến nghị sử dụng Feedly, Inoreader hoặc các trình đọc RSS khác để đăng ký trang web này.",
	[Key.rssCopied]: "Liên kết RSS đã được sao chép vào bảng tạm!",
	[Key.rssCopyFailed]:
		"Sao chép không thành công, vui lòng sao chép liên kết theo cách thủ công",

	// Trang Atom
	[Key.atom]: "Nguồn cấp dữ liệu Atom",
	[Key.atomDescription]: "Đăng ký để nhận thông tin cập nhật mới nhất",
	[Key.atomSubtitle]:
		"Đăng ký qua Atom để nhận bài viết và thông tin cập nhật mới nhất ngay lập tức",
	[Key.atomLink]: "Liên kết Atom",
	[Key.atomCopyToReader]: "Sao chép liên kết đến trình đọc Atom của bạn",
	[Key.atomCopyLink]: "Sao chép",
	[Key.atomLatestPosts]: "Bài viết mới nhất",
	[Key.atomWhatIsAtom]: "Atom là gì?",
	[Key.atomWhatIsAtomDescription]:
		"Atom (Định dạng Phân phối Atom) là một tiêu chuẩn dựa trên XML để mô tả các nguồn cấp dữ liệu và các mục của chúng. Với Atom, bạn có thể:",
	[Key.atomBenefit1]:
		"Nhận nội dung trang web mới nhất kịp thời mà không cần truy cập thủ công",
	[Key.atomBenefit2]: "Quản lý đăng ký nhiều trang web tại một nơi",

	[Key.atomBenefit3]: "Tránh bỏ lỡ các bản cập nhật và bài viết quan trọng",
	[Key.atomBenefit4]: "Tận hưởng trải nghiệm đọc sách sạch sẽ, không quảng cáo",
	[Key.atomHowToUse]:
		"Khuyến nghị sử dụng Feedly, Inoreader hoặc các trình đọc Atom khác để đăng ký trang web này.",

	[Key.atomCopied]: "Liên kết Atom đã được sao chép vào bảng tạm!",

	[Key.atomCopyFailed]:
		"Sao chép không thành công, vui lòng sao chép liên kết theo cách thủ công",

	// Chế độ hình nền
	[Key.wallpaperBanner]: "Chế độ biểu ngữ",
	[Key.wallpaperFullscreen]: "Chế độ toàn màn hình",
	[Key.wallpaperNone]: "Ẩn hình nền",

	// Trang Kỹ năng
	[Key.skills]: "Kỹ năng",
	[Key.skillsSubtitle]: "Kỹ năng và chuyên môn kỹ thuật của tôi",
	[Key.skillsFrontend]: "Phát triển Frontend",
	[Key.skillsBackend]: "Backend Phát triển",
	[Key.skillsDatabase]: "Cơ sở dữ liệu",
	[Key.skillsTools]: "Công cụ phát triển",
	[Key.skillsOther]: "Các kỹ năng khác",
	[Key.skillLevel]: "Thành thạo",
	[Key.skillLevelBeginner]: "Người mới bắt đầu",
	[Key.skillLevelIntermediate]: "Trung cấp",
	[Key.skillLevelAdvanced]: "Nâng cao",
	[Key.skillLevelExpert]: "Chuyên gia",
	[Key.skillExperience]: "Kinh nghiệm",
	[Key.skillYears]: "năm",
	[Key.skillMonths]: "tháng",
	[Key.skillsTotal]: "Tổng số kỹ năng",
	[Key.skillsExpert]: "Cấp độ chuyên gia",
	[Key.skillsAdvanced]: "Nâng cao",
	[Key.skillsIntermediate]: "Trung cấp",
	[Key.skillsBeginner]: "Người mới bắt đầu",
	[Key.skillsAdvancedTitle]: "Kỹ năng chuyên môn",
	[Key.skillsProjects]: "Dự án liên quan",
	[Key.skillsDistribution]: "Phân phối kỹ năng",
	[Key.skillsByLevel]: "Theo cấp độ",
	[Key.skillsByCategory]: "Theo danh mục",
	[Key.noData]: "Không có dữ liệu",

	// Trang Dòng thời gian
	[Key.timeline]: "Dòng thời gian",
	[Key.timelineSubtitle]:
		"Hành trình phát triển và những cột mốc quan trọng của tôi",
	[Key.timelineEducation]: "Giáo dục",
	[Key.timelineWork]: "Kinh nghiệm làm việc",
	[Key.timelineProject]: "Kinh nghiệm dự án",
	[Key.timelineAchievement]: "Thành tích",
	[Key.timelinePresent]: "Hiện tại",
	[Key.timelineLocation]: "Địa điểm",
	[Key.timelineDescription]: "Mô tả chi tiết",
	[Key.timelineMonths]: "tháng",
	[Key.timelineYears]: "năm",
	[Key.timelineTotal]: "Tổng cộng",
	[Key.timelineProjects]: "Dự án",
	[Key.timelineExperience]: "Kinh nghiệm làm việc",
	[Key.timelineCurrent]: "Trạng thái hiện tại",
	[Key.timelineHistory]: "Lịch sử",
	[Key.timelineAchievements]: "Thành tích",
	[Key.timelineStartDate]: "Ngày bắt đầu",
	[Key.timelineDuration]: "Thời gian",

	// Bảo vệ bằng mật khẩu
	[Key.passwordProtected]: "Đã bảo vệ bằng mật khẩu",
	[Key.passwordProtectedTitle]: "Nội dung này được bảo vệ bằng mật khẩu",
	[Key.passwordProtectedDescription]:
		"Vui lòng nhập mật khẩu để xem nội dung được bảo vệ",
	[Key.passwordPlaceholder]: "Nhập mật khẩu",
	[Key.passwordUnlock]: "Mở khóa",
	[Key.passwordUnlocking]: "Đang mở khóa...",
	[Key.passwordIncorrect]: "Mật khẩu không đúng, vui lòng thử lại",
	[Key.passwordDecryptError]:
		"Giải mã không thành công, vui lòng kiểm tra xem mật khẩu có đúng không",
	[Key.passwordRequired]: "Vui lòng nhập mật khẩu",
	[Key.passwordVerifying]: "Đang xác minh...",
	[Key.passwordDecryptFailed]:
		"Giải mã không thành công, vui lòng kiểm tra lại mật khẩu",
	[Key.passwordDecryptRetry]: "Giải mã không thành công, vui lòng thử lại",

	[Key.passwordUnlockButton]: "Mở khóa",
	[Key.copyFailed]: "Sao chép không thành công:",
	[Key.syntaxHighlightFailed]: "Làm nổi bật cú pháp không thành công:",
	[Key.autoSyntaxHighlightFailed]:
		"Tự động làm nổi bật cú pháp cũng không thành công:",

	[Key.decryptionError]: "Đã xảy ra lỗi trong quá trình giải mã:",

	// Thẻ chấm công sửa đổi lần cuối
	[Key.lastModifiedPrefix]: "Thời gian kể từ lần chỉnh sửa cuối cùng: ",
	[Key.lastModifiedOutdated]: "Một số thông tin có thể đã lỗi thời",
	[Key.year]: "y",
	[Key.month]: "m",
	[Key.day]: "d",
	[Key.hour]: "h",
	[Key.minute]: "min",
	[Key.second]: "s",

	// Thống kê trang web
	[Key.siteStats]: "Thống kê trang web",
	[Key.siteStatsPostCount]: "Bài đăng",
	[Key.siteStatsCategoryCount]: "Danh mục",
	[Key.siteStatsTagCount]: "Thẻ",
	[Key.siteStatsTotalWords]: "Tổng số từ",
	[Key.siteStatsRunningDays]: "Thời gian chạy",
	[Key.siteStatsLastUpdate]: "Hoạt động gần đây nhất",
	[Key.siteStatsDaysAgo]: "{days} ngày trước",
	[Key.siteStatsDays]: "{days} ngày",

	// Thành phần Lịch
	[Key.calendarSunday]: "Chủ Nhật",
	[Key.calendarMonday]: "Thứ Hai",
	[Key.calendarTuesday]: "Thứ Ba",
	[Key.calendarWednesday]: "Thứ Tư",
	[Key.calendarThursday]: "Thứ Năm",
	[Key.calendarFriday]: "Thứ Sáu",
	[Key.calendarSaturday]: "Thứ Bảy",
	[Key.calendarJanuary]: "Tháng Một",
	[Key.calendarFebruary]: "Tháng Hai",
	[Key.calendarMarch]: "Tháng Ba",
	[Key.calendarApril]: "Tháng Tư",
	[Key.calendarMay]: "Tháng Năm",
	[Key.calendarJune]: "Tháng Sáu",
	[Key.calendarJuly]: "Tháng Bảy",
	[Key.calendarAugust]: "Tháng Tám",
	[Key.calendarSeptember]: "Tháng Chín",
	[Key.calendarOctober]: "Tháng Mười",
	[Key.calendarNovember]: "Tháng Mười Một",
	[Key.calendarDecember]: "Tháng Mười Hai",

	// Chức năng Chia sẻ
	[Key.shareArticle]: "Chia sẻ",
	[Key.generatingPoster]: "Đang tạo poster...",
	[Key.copied]: "Đã sao chép",
	[Key.copyLink]: "Sao chép liên kết",
	[Key.savePoster]: "Lưu poster",
	[Key.scanToRead]: "Quét để đọc",
	[Key.shareOnSocial]: "Chia sẻ",
	[Key.shareOnSocialDescription]:
		"Nếu bài viết này hữu ích với bạn, hãy chia sẻ với người khác!",

	// Thống kê Trang cá nhân
	[Key.profileStatsLoading]: "Đang tải thống kê...",
	[Key.profileStatsPageViews]: "Lượt xem trang",
	[Key.profileStatsVisits]: "Lượt truy cập",
	[Key.profileStatsUnavailable]: "Thống kê không khả dụng",

	// Nút Chuyển đổi Bố cục
	[Key.switchToGridMode]: "Chuyển sang Chế độ Lưới",
	[Key.switchToListMode]: "Chuyển sang Chế độ Danh sách Cách thức",
};
